    ...
    <div class="flex w-full h-full flex-col md:flex-row {{ app()->getLocale() === 'ar' ? 'md:flex-row-reverse' : '' }}">
        ...
        <div class="md:w-1/2 w-full flex items-center justify-center bg-white p-8 md:p-12">
            <div class="w-full max-w-md space-y-8">
                ...
                <form class="space-y-6" action="{{ route('pay.form.redirect', $course) }}" method="POST">
                    @csrf

                    <!-- Hidden Days Inputs -->
                    @foreach ($data['days'] ?? [] as $dayName => $dayData)
                        <input type="hidden" name="days[{{ $dayName }}][id]" value="{{ $dayData['id'] }}">
                        <input type="hidden" name="days[{{ $dayName }}][start_time]"
                    ...
                    <div class="space-y-2">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            <i
                                class="fas fa-envelope {{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }} text-[#79131d]"></i>
                            {{ __('messages.email') }}
                        </label>
                        <div class="relative">
                            ...
                            <i
                                class="fas fa-envelope absolute {{ app()->getLocale() === 'ar' ? 'right-4' : 'left-4' }} top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                        </div>
                        @error('email')
                    ...
                    <div class="space-y-2">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            <i
                                class="fas fa-lock {{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }} text-[#79131d]"></i>
                        ...
                        <div class="relative">
                            ...
                            <i
                                class="fas fa-lock absolute {{ app()->getLocale() === 'ar' ? 'right-4' : 'left-4' }} top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                            <button type="button" onclick="togglePassword()"
                                class="absolute {{ app()->getLocale() === 'ar' ? 'left-4' : 'right-4' }} top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-[#79131d] transition">
                ...
                <div class="text-center pt-4 border-t border-gray-200">
                    <p class="text-sm text-gray-600">
                        ...
                        <a href="{{ route('register') }}"
                            class="text-[#79131d] font-semibold hover:text-[#5a0f16] hover:underline transition">
