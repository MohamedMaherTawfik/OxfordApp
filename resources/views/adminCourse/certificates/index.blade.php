@php
    ...
    $assined = assignedCertificates::get();
    // dd($assined);
@endphp
...
<x-panel>
    ...
    <div class="max-w-2xl mx-auto p-6 mt-3 mb-3 rounded-lg shadow" dir="rtl">
        <h2 class="text-xl font-semibold mb-4"> {{ __('messages.courseCertificates') }} {{ $course->title }}</h2>

        <div class="text-right">
            <a href="{{ route('admin.certificates.create', ['slug' => $course->slug, 'id' => auth()->id()]) }}"
                class="inline-block px-4 py-2 bg-[#79131d] text-white rounded-md hover:bg-[#5e1017] transition">
                {{ __('messages.createCertificates') }} By {{ auth()->user()->name }}
            </a>
        </div>
    </div>
