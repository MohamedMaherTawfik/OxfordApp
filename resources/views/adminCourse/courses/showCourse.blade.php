        ...
        <main class="flex-1 overflow-y-auto p-6 bg-gray-50">
            <div class="mb-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-800">{{ __('messages.overview') }}</h1>
                        <p class="text-gray-600">
                            {{ __('messages.Welcomeback') }}<span class="font-bold">{{ Auth::user()->name }}</span>.
                            {{ __('messages.happen') }}
                        </p>
                    </div>
                    ...
                    <div x-data="{ dropdownOpen: false, modalOpen: false }" class="relative inline-block text-left">
                        <button @click="dropdownOpen = !dropdownOpen"
                            class="bg-[#79131DC0] text-white px-4 py-2 ml-20 rounded-lg shadow hover:bg-[#79131d]">
                            {{ __('messages.option') }}
                        </button>
                        ...
                        <div x-show="dropdownOpen" @click.away="dropdownOpen = false"
                            ...
                            <a href="{{ route('admin.lessons.create', $course->slug) }}"
                                class="block px-4 py-2 text-gray-700 hover:bg-gray-100">{{ __('messages.add_lesson') }}</a>
                            <a href="{{ route('admin.courses.edit', $course->slug) }}"
                                class="block px-4 py-2 text-gray-700 hover:bg-gray-100">{{ __('messages.edit_lesson') }}</a>
                            ...
                            <a href="{{ route('admin.zoom.index', $course) }}"
                                class="block px-4 py-2 text-gray-700 hover:bg-gray-100">{{ __('teacher.meetings') }}</a>
                            <a href="{{ route('admin.certificates.index', $course->slug) }}"
                                class="block px-4 py-2 text-gray-700 hover:bg-gray-100">{{ __('teacher.certificate') }}</a>
                            <a href="{{ route('admin.course-schedules.index', $course) }}"
                                class="block px-4 py-2 text-gray-700 hover:bg-gray-100">{{ __('teacher.add_schedule') }}</a>
                                ...
                                <form method="POST" action="{{ route('admin.courses.notify', $course) }}">
                                    ...
                                    <div class="mb-4">
                                        <label for="message" class="block text-gray-700">
                                            {{ __('teacher.send_notification') }}</label>
                                    ...
                                    <div class="flex justify-end space-x-2">
                                        <button type="button" @click="modalOpen = false"
                                            ...
                                            {{ __('teacher.cancel') }}</button>
                                        <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded">
                                            {{ __('teacher.send') }}</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                ...
                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-semibold text-gray-800">{{ __('teacher.riyal') }}
                                {{ $price }}</p>
                        </div>
