        ...
        <div class="max-w-2xl w-full bg-white p-8 rounded-lg shadow-md">
            <form method="POST" action="{{ route('admin.courses.store') }}" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Level -->
                    <div>
                        <label for="level" class="block text-sm font-medium text-gray-700">{{ __('messages.level') }}</label>
                        <select id="level" name="level"
                            ...
                            <option value="Beginner" {{ old('level') == 'beginner' ? 'selected' : '' }}>{{ __('messages.Beginner') }}
                            </option>
                            <option value="Mid" {{ old('level') == 'Mid' ? 'selected' : '' }}>
                                {{ __('messages.mid') }}</option>
                            <option value="Advanced" {{ old('level') == 'advanced' ? 'selected' : '' }}>{{ __('messages.advanced') }}
                            </option>
                        </select>
                        @error('level')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
