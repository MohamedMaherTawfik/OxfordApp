            ...
            <div class="flex flex-wrap justify-center gap-3 {{ $isArabic ? 'flex-row-reverse' : 'flex-row' }}">
                @foreach ($categories->take(4) as $item)
                    <a href="{{ route('gate.diplomas.categorey.show', $item->slug) }}"
                        class="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-full text-sm font-medium text-gray-700 hover:bg-gray-50 transition duration-200">
