<nav class="bg-white shadow-sm z-50 fixed top-0 w-full" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}"
    ...
    <!-- Top Section -->
    <div class="container mx-auto px-6 py-1 flex items-center justify-between {{ app()->getLocale() === 'ar' ? 'flex-row-reverse' : '' }}">
            ...
            <div class="relative" x-data="{ langMenu: false }">
                ...
                <div x-show="langMenu" 
                    ...
                    <a href="{{ route('lang.switch', ['locale' => 'en']) }}"
                        ...
                        <span>English</span>
                        @if(app()->getLocale() === 'en')
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 {{ app()->getLocale() === 'ar' ? 'ml-auto' : 'mr-auto' }}" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="hidden md:flex flex-wrap justify-center gap-2 font-semibold text-[17px]">
            ...
            @if (Auth::check())
                <a href="{{ route('myCourses') }}" class="px-4 py-2 rounded-lg text-gray-700 hover:text-[#79131d] hover:bg-gray-50 transition-all duration-200">
                    {{ __('messages.MyCourses') }}
                </a>
            @endif

            @guest
                <a href="{{ route('register') }}" class="px-4 py-2 rounded-lg text-gray-700 hover:text-[#79131d] hover:bg-gray-50 transition-all duration-200">
