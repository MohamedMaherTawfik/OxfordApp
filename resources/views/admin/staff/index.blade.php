    ...
    <div class="p-6 bg-gray-50 min-h-screen">
                                    ...
                                    <div class="flex flex-wrap gap-1 justify-center">
                                        @if($member->staffPermissions)
                                            @if($member->staffPermissions->manage_users)
                                                <span class="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded">المستخدمين</span>
                                            @endif
                                            @if($member->staffPermissions->manage_courses)
                                                <span class="px-2 py-1 text-xs bg-green-100 text-green-800 rounded">الدورات</span>
                                            @endif
                                            @if($member->staffPermissions->manage_teachers)
                                                <span class="px-2 py-1 text-xs bg-purple-100 text-purple-800 rounded">المعلمين</span>
                                            @endif
                                            @if($member->staffPermissions->manage_payments)
                                                <span class="px-2 py-1 text-xs bg-yellow-100 text-yellow-800 rounded">المدفوعات</span>
                                            @endif
                                        @else
        ...
        @else
            <div class="bg-white rounded-lg shadow p-12 text-center">
                ...
                <p class="text-gray-600 text-lg">لا يوجد موظفين حالياً</p>
                <a href="{{ route('admin.staff.create') }}" 
